(function () {
  ...
  class BodyProcessors {
    constructor() {
      __privateAdd(this, _registeredProcessors, []);
      __privateGet(this, _registeredProcessors).push(...constructors.map((c) => new c()));
    }
    find(name) {
      return __privateGet(this, _registeredProcessors).find(
    ...
    getProcessors() {
      return __privateGet(this, _registeredProcessors);
    }
    getNames() {
      return __privateGet(this, _registeredProcessors).map((processor) => processor.getName());
    }
    getDefaultProcessor() {
      return __privateGet(this, _registeredProcessors).at(0);
    }
  ...
  const messageListener = async (message) => {
    ...
    try {
      ...
      const formPrototype = Object.getPrototypeOf(form);
      ...
      violationListener = (event) => {
        ...
        const violatedPolicy = event.originalPolicy.match(`${event.violatedDirective}[^;]+`).toString();
        const errorMessage = `Failed to send data to '${event.blockedURI}' because it violates the following Content Security Policy directive: "${violatedPolicy}". You can execute the request again after going to a page without directive "${violatedPolicy}". For example, https://www.google.com`;
